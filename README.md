...
This file covers running the tool. [docs/METHOD.md](docs/METHOD.md) covers what each check is
doing and why it is shaped that way, which is mostly a record of how earlier versions were
wrong. `CHANGELOG.md` has the dated list.

Most readers need one of three things, and not the rest of it.

| You want to | Read |
|---|---|
| Reproduce the article's numbers | [Quick start](#quick-start), then [What `make verify` compares](#what-make-verify-compares) |
| Check your own monitoring data | [Check your own data](#check-your-own-data). Two CSVs, no download, about two minutes |
| Put your own detector through the procedure | [Run against your own detector](#run-against-your-own-detector) |
...
| `make test` | 15 s | 231 unit tests for the check path |
...
| Flag-everything guard on the alerted rate. Fires when the alerted rate squared reaches twice prevalence, so a detector may alert often, or far more often than incidents occur, but not both | 8b | `fdes/checks.py::alert_rate_saturated`, `alert_rate_far_above_prevalence`, `make check` only |
| Alert volume, as alerts per incident and per day. Nothing else here measures how often a detector pages | this tool's own check | `fdes/byod.py::alerts_per_incident`, `make check` only |
...
rounding-level distance outside the margin. The rule, the thresholds and the export that
motivated each of them are in [docs/METHOD.md](docs/METHOD.md). That reading is applied
...
Every rule in this path exists because a real export broke an earlier version of it. If a
verdict here surprises you, [docs/METHOD.md](docs/METHOD.md) explains what the check is
measuring, what it got wrong before, and what it still cannot tell you.
...
docs/METHOD.md          what each check does and why it is shaped that way
...
@software{anjum2026otelreproduction,
  ...
  publisher = {Zenodo},
  year      = {2026},
  doi       = {10.5281/zenodo.22313315}
}
```

This reproduction package is archived at Zenodo, DOI
[10.5281/zenodo.22313315](https://doi.org/10.5281/zenodo.22313315) (v1.3.9, concept DOI
